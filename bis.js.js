...
function updateChatTitle() {
    if (currentMessages.length > 0) {
        var firstMsg = currentMessages[0];
        if (firstMsg && firstMsg.type === 'user') {
            var title = firstMsg.text.substring(0, 40) + (firstMsg.text.length > 40 ? '...' : '');
...
if (getStartedBtn) {
    getStartedBtn.addEventListener("click", function() {
        showMainPage();
        // Load chat history when entering main page
        loadChatHistory();
        if (chatHistory.length === 0) {
            createNewChat();
        } else {
...
function addMessage(type, content, sources, checklist) {
    ...
    if (type === 'user' && currentMessages.length === 1) {
        updateChatTitle();
    }
}
...
console.log("💡 Tip: Edit BIS_KNOWLEDGE to add more products");
